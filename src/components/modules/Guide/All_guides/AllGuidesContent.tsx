...
export default function AllGuidesContent({ guides, pagination, filterOptions }: AllGuidesContentProps) {
  ...
  const handleSearch = useCallback((searchTerm: string) => updateURL({ searchTerm }), [updateURL]);
  const handleFilterChange = useCallback((newFilters: Record<string, any>) => updateURL(newFilters), [updateURL]);
  const handlePageChange = useCallback((page: number) => {
    updateURL({ page });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [updateURL]);
  const clearFilters = useCallback(() => router.push('/all-guides'), [router]);
    ...
    <div className="container mx-auto px-4 py-10">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Explore verified local guides, connect, and create unforgettable travel experiences.
        </p>
      </div>
      ...
      <div className="mb-6">
        <GuideSearchBar
          ...
          initialValue={currentFilters.searchTerm}
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        {/* Filters Sidebar */}
        <aside className={`${showFilters ? 'block' : 'hidden'} lg:block lg:w-80`}>
        ...
        <main className="flex-1">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div className="flex items-center gap-2">
              <Label className="text-sm font-semibold text-gray-900 mb-0">Sort By:</Label>
              <SelectFilter
                paramName="sort"
                placeholder="Sort by"
                options={[
