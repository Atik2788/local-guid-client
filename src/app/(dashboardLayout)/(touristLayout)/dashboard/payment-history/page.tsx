...
type PaymentHistoryPageParams = Record<string, string | string[] | undefined>;

const PaymentHistoryPage = async ({
  searchParams,
}: {
  searchParams?: PaymentHistoryPageParams;
}) => {
  const user = await getUserInfo();
  ...
  if (searchParams?.searchTerm) filters.searchTerm = searchParams.searchTerm as string;
  if (searchParams?.status && searchParams.status !== "all") filters.status = searchParams.status as string;
